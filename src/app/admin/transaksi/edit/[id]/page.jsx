...
import { useGetOneTransaksi, useUpdateTransaksi } from "@/hooks/useTransaksi";
...
export default function EditTransaksiPage({ params }) {
  ...
  const { data: dataTransaksi } = useGetOneTransaksi({
    idTransaksi,
  });

  useEffect(() => {
    if (dataTransaksi?.data?.data) {
      const res = dataTransaksi.data.data;

      setBarang(res.transaksiDetail);
      setNomorTransaksi(res.nomorTransaksi);
      setTanggalTransaksi(new Date(res.tanggalTransaksi));
      setCustomer({
        id: res.customer.id,
      });
    }
  ...
  useEffect(() => {
    const handleCounter = async () => {
      // Ensure tanggalTransaksi is a Date object
      // const transactionDate = new Date(tanggalTransaksi);

      if (dataTransaksi?.data?.data?.tanggalTransaksi) {
        const transactionDate = tanggalTransaksi;
        ...
        if (!isNaN(transactionDate)) {
          ...
          // Check if nomorTransaksi matches the current month and year
          const currentYearMonth = `${year}-${month}`;
          const existingYearMonth =
            dataTransaksi.data.data.nomorTransaksi.slice(3, 10);

          console.log("current year", currentYearMonth);
          console.log("existing", existingYearMonth);
          ...
          if (existingYearMonth !== currentYearMonth) {
            ...
            const newNomorTransaksi = `SO/${year}-${month}/${String(
              newCounter
            ).padStart(4, "0")}`;

            setNomorTransaksi(newNomorTransaksi);
          } else {
            setNomorTransaksi(dataTransaksi.data.data.nomorTransaksi);
          }
        } else {
          console.error("Invalid Date:", tanggalTransaksi);
        }
      }
    };

    handleCounter();
  }, [tanggalTransaksi, dataTransaksi]);
  ...
  const { mutate } = useUpdateTransaksi({
    onError: () => {
      toast({
